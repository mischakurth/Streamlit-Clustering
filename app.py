...
if data_source == "Generieren":
    ...
    import_data = st.sidebar.text_area("Daten eingeben", "[[0, 0], [1, 1], [0, 1]]")
    if st.sidebar.button("Daten importieren"):
        try:
            ...
            if X.ndim != 2 or X.shape[1] != 2:
                ...
                st.session_state['X'] = X
                st.session_state['y'] = np.zeros(X.shape[0])
                st.session_state['data_generated'] = True
...
if algo_name == "K-Means":
    ...
    if col_start.button("Initialisieren"):
        if st.session_state['data_generated']:
            try:
                algo = KMeansManual(k=k, max_iter=max_iter) # k is from closure/current scope which is fine here
